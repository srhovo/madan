...
> **已退役的脚本不在本目录**：`legacy/` 子目录下的那份"上游"全链路脚本基于
> 8.2.22 编写，在懒加载 chunk 架构引入后已失去回归对照能力，**不再维护、也不参与
> run-all.sh**。把它单独放进 `legacy/` 而不是留在本目录，是为了让"这个已退役"
> 从路径上就能看出来 —— 不必靠记文件名或翻这份文档去分辨。新测试一律用本目录里
> 的 8.3 架构版。
...
上游原版作为**历史参照**保留在 `tests/legacy/` 子目录，不再维护、不参与
run-all.sh。放到子目录里是为了让"已退役"这件事从路径上一眼可见 ——
原先它和现役的"8.3架构版"并排放在同一目录、文件名又高度相似，
只靠这份文档说明谁是现役的，太依赖记忆。新测试请用 8.3 架构版。
