/**
 * 端到端 OTA 行为验证 —— 用「从 madan-<当前版本>.zip 真实解压出来的 index.html」
 ...
 *
 * 版本自适应：目标版本取自 version.json，包名与断言值都由它推导 ——
 * 此前脚本把 '8.3.30' 写死在路径和 4 处断言里，导致版本一升就失效
 * （实测：包内 8.3.30 对云端 8.3.36，B1 断言必然失败）。
 * 现在它跟随当前版本，不再需要随每次发版手改。
 *
 * 用法：node tests/ota-e2e.js
 * 退出码：0 全部通过 / 1 有断言失败
 */
const fs = require('fs');
...
const ZIP = path.join(ROOT, `madan-${VER}.zip`);

if (!fs.existsSync(ZIP)) {
  console.log('--- OTA 端到端 ---');
  console.log(`  ✗ 找不到 ${path.basename(ZIP)} —— 本检查需要一个与 version.json 对齐的真实包。`);
  console.log('     发版后本文件必然存在；若在纯代码开发中途运行，请先发版或跳过本检查。');
  process.exit(1);
}
...
setTimeout(() => {
  ...
  ck(`A1 包内 index.html 的版本号 = ${VER}`, got === VER, `实际 "${got}"`);
  ck('A2 notifyAppReady 被调用（原生层不会误判包不健康）', readyCalled);
  ck('A3 零未捕获异常', errors.length === 0, errors.slice(0, 2).join(' | '));
  ck(`B1 本地=云端=${VER} 时不再重复下载`, downloads.length === 0, `下载了 ${JSON.stringify(downloads)}`);
